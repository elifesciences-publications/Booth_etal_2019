...
genes <- read.delim("Results/DEseq/Groups/N3UvF3U.txt", 
...
N2 <- genes[which(genes$padj < 0.05 & genes$log2FoldChange > 0), ] #2531 N2 enriched
CEH18.N <- N2[which(rownames(N2) %in% CEH18.all), ] #204
...
write.table(my.dataOrdered, 
            ...
            quote = FALSE,
            sep = "\t",
            file="Results/DEseq/Groups/GO/N3UvF3U-N3Uup-withCEH18peak.txt")



########################################################################
# Extract genes that are expressed more highly in fem-1 and that are associated with a CEH-18 peak
fem <- genes[which(genes$padj < 0.05 & genes$log2FoldChange < 0), ] #2322 fem-1 enriched
CEH18.F <- fem[which(rownames(fem) %in% CEH18.all), ] #209
...
write.table(my.dataOrdered, 
            ...
            quote = FALSE,
            sep = "\t",
            file="Results/DEseq/Groups/GO/N3UvF3U-F3Uup-withCEH18peak.txt")
