...
deg.down <- genes[which(genes$padj < 0.05 & genes$log2FoldChange < 0), ] #2322 fem-1 enriched (F3U)
...
write.table(my.data.downOrdered, 
            ...
            quote = FALSE,
            sep = "\t",
            file="Results/DEseq/Groups/GO/N3UvF3U-fem-1UP.txt")

#####################################################################################################
# Extract genes that are UP in N3M
deg.up <- genes[which(genes$padj < 0.05 & genes$log2FoldChange > 0),] #2531 UP in N3U
...
write.table(my.data.upOrdered, 
            ...
            quote = FALSE,
            sep = "\t",
            file="Results/DEseq/Groups/GO/N3UvF3U-N2UP.txt")
