...
The folder “Results” and the folders within it are empty intentionally and are used for the paths in the scripts to output the data.
...
3. Descriptive-*.R
	This script performs principal component analysis and generates PCA plots and a dendrogram of the data. There are three versions of this script for creating a PCA of all the data (Fig. S3B) and for the two PCAs displayed in Fig. 2E, F. The colors shown in the manuscript differ slightly from the plots made here because the coloring was modified in Illustrator.

4. Model_with_Groups.R
	This script uses DEseq2 to calculate differential expression. The values calculated here are used in GO_term_enrichment_*.R and are included in a supplemental table.

5. GO_term_enrichment_*.R
	This set of scripts calculates GO term enrichment for the gene sets of interest. For figure 2E, F, the two PCA scripts were used and take the top 5% of genes driving the positive and negative PC1 and PC2 (using the data generated by Descriptive-*.R). The other two scripts use the differential expression calculations from Model_with_Groups.R to generate the data displayed in Figure S3C, D, the supplemental data. The bar plots in supplemental figure 3 were generated with Prism 7 using -log10 transformed data from this script.
